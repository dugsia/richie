...
def site_metas(request):
    ...
    authentication_backend = getattr(settings, "AUTHENTICATION_BACKEND", {})
    context = {
        **{
                ...
                [
                    {
                        ...
                        "action": str(
                            url["href"].format(
                                base_url=authentication_backend["BASE_URL"]
                            )
                        ),
                    }
                    for url in authentication_backend["PROFILE_URLS"]
                ]
            ),
        },
        "FRONTEND_CONTEXT": json.dumps(
            {
                "context": {
                    ...
                    "authentication": {
                        "endpoint": authentication_backend["BASE_URL"],
                        "backend": authentication_backend["BACKEND"],
                    },
                    "lms_backends": [
                        {
